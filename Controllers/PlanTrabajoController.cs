...
public PlanTrabajoController(SemilleroContext context)
{
...
_context = context;
if (_context.Trabajo.Count() == 0)
{

    // _context.Trabajo.Add(new PlanTrabajoItem {Nombrefacultad = "Facultad de ingenerias y tecnologias ",  Programa = null});
...
[HttpGet]
public async Task<ActionResult<IEnumerable<PlantrabajoItem>>> GetPacienteItems()
{
...
[HttpGet("{id}")]
public async Task<ActionResult<PlantrabajoItem>> GetPacienteItem(int id)
{
...
// }
[HttpPost]
public async Task<ActionResult<PlantrabajoItem>> PostPacienteItem(PlantrabajoItem item)
{
   
         // calcularEstudiante(item);
_context.Trabajo.Add(item);
